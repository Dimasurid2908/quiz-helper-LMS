...
// @grant        none
// @downloadURL https://update.greasyfork.org/scripts/501725/Quiz%20Helper.user.js
// @updateURL https://update.greasyfork.org/scripts/501725/Quiz%20Helper.meta.js
// ==/UserScript==
...
// Mảng chứa các key premium hợp lệ
const validPremiumKeys = ['LAPANGAY', 'IMGAY', 'PREMIUM2908', 'SPECIALKEY'];
...
function updateScoresAndTotals() {
    ...
    // Update the semester percentages
    let semester1Cell = document.querySelector('tfoot tr:nth-child(1) td:nth-child(2)');
    let semester2Cell = document.querySelector('tfoot tr:nth-child(2) td:nth-child(2)');
    let overallPercentageCell = document.querySelector('tfoot tr:nth-child(3) td:nth-child(2)');
    if (semester1Cell) {
        semester1Cell.innerText = '100.00%';
    }
    if (semester2Cell) {
        semester2Cell.innerText = '100.00%';
    }
    if (overallPercentageCell) {
        overallPercentageCell.innerText = '100.00%';
    }
    // Update the "Tổng cộng" row if it's not displaying the number
...
function saveToggleState(isChecked) {
    localStorage.setItem('scoreUpdateToggleState', isChecked);
}

// Function to load toggle state
function loadToggleState() {
    return localStorage.getItem('scoreUpdateToggleState') === 'true';
}
...
// Add event listener for the score update toggle
document.getElementById('scoreUpdateToggle').addEventListener('change', function() {
    if (this.checked) {
        enableScoreUpdate();
    } else {
        disableScoreUpdate();
...
document.addEventListener('DOMContentLoaded', function() {
    const scoreUpdateToggle = document.getElementById('scoreUpdateToggle');
    const savedState = loadToggleState();
    scoreUpdateToggle.checked = savedState;
    if (savedState) {
        enableScoreUpdate();
    }
});
// Add event listener for the Copy/Paste toggle
document.getElementById('copyPasteToggle').addEventListener('change', function() {
    ...
    if (this.checked) {
        ...
        alert('Chức năng Copy và Paste đã được bật!');
    } else {
        ...
        alert('Chức năng Copy và Paste đã được tắt!');
    }
});
// Key system functions
function getFreeKey() {
    document.getElementById('getFreeKeyButton').disabled = true;
    document.getElementById('freeKeyTimer').style.display = 'block';
    let countdown = 60;
    const countdownElement = document.getElementById('freeKeyCountdown');
    const countdownInterval = setInterval(() => {
        ...
        if (countdown <= 0) {
            ...
            document.getElementById('freeKeyInput').value = freeKey;
            ...
            document.getElementById('freeKeyTimer').style.display = 'none';
            document.getElementById('getFreeKeyButton').disabled = false;
        }
    }, 1000);
}
...
function activateFreeKey() {
    ...
    if (isValidFreeKey(inputKey)) {
        activeKey = 'FREE';
        keyExpirationTime = Date.now() + 1800000; // 30 minutes from now
...
function activatePremiumKey() {
    ...
    if (isValidPremiumKey(inputKey)) {
        activeKey = 'PREMIUM';
        keyExpirationTime = null; // Premium key không hết hạn
        localStorage.setItem('activeKey', activeKey);
        localStorage.removeItem('keyExpirationTime');
        showFunctions(true);
        alert('Chúc mừng! Bạn đã kích hoạt Premium Key. Tận hưởng đầy đủ tính năng không giới hạn!');
    } else {
        alert('Premium Key không hợp lệ! Hãy liên hệ chúng tôi để mua Premium Key chính hãng.');
    }
}

function showFunctions(isPremium) {
    ...
    buttons.forEach(button => {
        if (isPremium || ['extractButton', 'selectAnswersButton', 'startTimerButton'].includes(button.id)) {
            ...
            button.onclick = showPremiumAlert; // Add click handler for premium alert
        }
    });

    inputs.forEach(input => {
        if (isPremium || ['answersInput', 'timerInput'].includes(input.id)) {
            input.disabled = false;
        } else {
            input.disabled = true;
            input.onclick = showPremiumAlert; // Add click handler for premium alert
        }
    });
}
...
document.getElementById('contactButton').addEventListener('click', () => {
    alert('Vui lòng gửi email tới: minh102368@stu.vinschool.edu.vn để nhận thông tin về ưu đãi đặc biệt!');
});

// Check key validity on load
window.addEventListener('load', checkKeyValidity);
